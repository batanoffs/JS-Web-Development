...
export function createView() {
    render(createViewTemp(), document.querySelector(".container"));
}
...
async function submitHandler(event) {
    ...
    let { make, model, year, description, price, img, material } =
        Object.fromEntries(formData);
    
    // TODO: Continue with the rest of your code
    debugger;
    if (make.length < 4) {
        error.make = true;
        hasError = true;
    ...
    if (hasError) {
        return render(
            createViewTemp(error),
            document.querySelector(".container")
        );
    }

    await server.post(path.all, {
        make,
        model,
        year,
        description,
