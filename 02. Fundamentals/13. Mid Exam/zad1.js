function experienceGain(data) {
   ...
   for (let index = 0; index < numberOfBattles; index++) {
      ...
      if (battleCounter % 3 === 0) {
         ...
         currentExperience = currentExperience - 0.1 * currentExperience;
         gainedExp += currentExperience;
      ...
      if (amountExpRequired <= gainedExp) {
         console.log(
            `Player successfully collected his needed experience for ${battleCounter} battles.`
         );
         return;
      }
   }
   ...
   console.log(
      `Player was not able to collect the needed experience, ${neededExp.toFixed(
         2
      )} more needed.`
   );
}
